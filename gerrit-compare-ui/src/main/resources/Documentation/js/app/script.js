(function() {
	...
	var toggleCodeReviewViews = document
			.getElementsByClassName('controlScreen'); // :  Getting Code Review Section Class
	...
	vrSplitter.onclick = function() {
		patchesSection.className = (patchesSection.className == 'expandTree') ? 'collapseTree'
				: 'expandTree';
	}
	...
	function splitterAction(targetStateDiv) {
		...
		checkSpliters.className = (checkSpliters.className == 'showContent') ? 'hideContent'
				: 'showContent';
	}
...
function init(anchor) {
	...
	structuralDiff.getChangeIdDetails(changeID);
...
function loadGerritCommit() {
	structuralDiff.getGerritCommitDetails();
}
...
function renderPatchTree(getFinalPatchId) {
				...
				$(this).parents("tr").droppable(
						{
							...
							drop : function(e, ui) {
								...
								$(patchIdSelector).treetable("move",
										droppedEl.data("ttId"),
										$(this).data("ttId"));
							},
							hoverClass : "accept",
							over : function(e, ui) {
								...
								if (this != droppedEl[0]
										&& !$(this).is(".expanded")) {
									$(patchIdSelector).treetable("expandNode",
											$(this).data("ttId"));
								}
							}
						});
	...
	$("select.dropdown").css("opacity", .5).show().find(" + .dropdown").wrap(
			"<div />");

	// Code for the stylesheet switcher
	var stylesheets = $("link"), stylesLink = $("#stylesLink span");

	$("select[name='styles']").hide().change(
			function() {
				var active = stylesheets.not(
						stylesheets.not("[title='" + $(this).val() + "']")
								.attr("disabled", "disabled")).removeAttr(
						"disabled"), href = active.attr("href");
				if (!href) {
					stylesLink.text("inline styles");
				} else {
					stylesLink.empty().append(
							$(document.createElement("a")).text(href).attr(
									"href", href));
				}
			});

	var tableLength = $(patchIdSelector + '> tbody').children().each(
			function() {
...
(function() {
	...
	var fileSearchError = {
		...
		"errorHide" : function(errorBoxDelay, errorContent, overlayDelay,
				errorOverlay) {
			return $(this.fileSearchErrorSelector).delay(errorBoxDelay)
					.fadeOut(errorContent), $(this.overlay).delay(overlayDelay)
					.fadeOut(errorOverlay);
		},

		// Hiding the Error Box with Overlay
		"errorShow" : function(errorBoxDelay, errorContent, overlayDelay,
				errorOverlay) {
			return $(this.fileSearchErrorSelector).delay(errorBoxDelay).fadeIn(
					errorContent), $(this.overlay).delay(overlayDelay).fadeIn(
					errorOverlay);
		}
	...
	function searchFile() {
		...
		if (getValue.length !== 0) {
			...
			$(patch.getPatchId() + " span.file:contains('" + getValue + "')")
					.each(
							function() {

								collectSearches.push($(this).parents('tr')
										.attr('data-tt-id'));
							})
	...
	function patchTreeResult(getDataIdCollection) {
		...
		for (fileDataId = 0; fileDataId < getDataIdCollection.length; fileDataId++) {
			...
			for (fileParentsDataId = 0; fileParentsDataId < splitedFileDataId.length; fileParentsDataId++) {
				...
				fileParentRefinedDataId = fileParentRefinedDataId
						+ ((fileParentsDataId === 0) ? '' : '-')
						+ splitedFileDataId[fileParentsDataId];
				$(patch.getPatchId()).treetable('expandNode',
						fileParentRefinedDataId);
				$(
						patch.getPatchId() + " tr[data-tt-id ='"
								+ fileParentRefinedDataId + "']").attr(
						'data-add-tr', 'showThisTr');
				$(patch.getPatchId() + " tr[data-add-tr!= 'showThisTr']").attr(
						'data-add-tr', 'hideThisTr');
			}
